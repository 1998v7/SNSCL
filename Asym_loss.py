...
class NCELoss(nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        # label_one_hot = F.one_hot(labels, self.num_classes).float().to(pred.device)
        loss = -1 * torch.sum(labels * pred, dim=1) / (-pred.sum(dim=1))
        return self.scale * loss.mean()
...
class AUELoss(nn.Module):
    ...
    def forward(self, pred, labels):
        pred = F.softmax(pred, dim=1)
        # label_one_hot = F.one_hot(labels, self.num_classes).float().to(pred.device)
        loss = (torch.pow(self.a - torch.sum(labels * pred, dim=1), self.q) - (self.a-1)**self.q)/ self.q
        return loss.mean() * self.scale
...
class ANormLoss(nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        # label_one_hot = F.one_hot(labels, self.num_classes).float().to(pred.device)
        loss = torch.sum(torch.pow(torch.abs(self.a * labels-pred), self.p), dim=1) - (self.a-1)**self.p
...
class AExpLoss(torch.nn.Module):
    ...
    def forward(self, pred, labels):
        pred = F.softmax(pred, dim=1)
        # label_one_hot = F.one_hot(labels, self.num_classes).float().to(pred.device)
        loss = torch.exp(-torch.sum(labels * pred, dim=1) / self.a)
        return loss.mean() * self.scale
